// Minimal dependency-free SVG scatter plot for the efficient frontier
...
function scaleFns(points, minVol, maxSharpePoint) {
  ...
  const ys = points.map((p) => p.target_return).concat([minVol.return, maxSharpePoint.return]);
...
export default function FrontierChart({ frontier }) {
  if (!frontier || !frontier.points || !frontier.points.length) return null;
  const { points, min_volatility: minVol, max_sharpe: maxSharpe } = frontier;
  const { sx, sy, xMax, yLo, yHi } = scaleFns(points, minVol, maxSharpe);
  const path = points
    .slice()
    .sort((a, b) => a.volatility - b.volatility)
    .map((p, i) => `${i === 0 ? "M" : "L"}${sx(p.volatility)},${sy(p.target_return)}`)
    .join(" ");

  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="frontier-svg" role="img" aria-label="Efficient frontier chart">
      {/* axes */}
      ...
      {points.map((p, i) => (
        <circle key={i} cx={sx(p.volatility)} cy={sy(p.target_return)} r="2.5"
          fill="var(--color-brand)" opacity="0.55" />
      ))}
      ...
      <text x={sx(minVol.volatility) + 8} y={sy(minVol.return) - 8} className="chart-label">
        min-vol ({pct(minVol.volatility)}, {pct(minVol.return)})
      </text>
      ...
      <text x={sx(maxSharpe.volatility) + 8} y={sy(maxSharpe.return) + 16} className="chart-label">
        max-Sharpe ({pct(maxSharpe.volatility)}, {pct(maxSharpe.return)})
      </text>
    </svg>
  );
}
