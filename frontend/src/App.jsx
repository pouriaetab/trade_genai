...
export default function App() {
  ...
  // Load registry + project memory once.
  useEffect(() => {
  ...
  function persist(next = {}) {
    const chat = (next.chat || conversation).map((m) => ({
      role: m.role, content: m.content, meta: m.meta,
      execSummary: m.execSummary, isCode: m.isCode,
    })); // drop heavy execution payload (figures/HTML) from storage
    put(`/api/v1/projects/${PROJECT_ID}`, {
      notes: next.notes ?? notes, chat, cells: [],
    });
  }
  ...
  return (
    <>
      ...
      <main>
          ...
          <div className="learn">
            ...
            <div className="col">
              <ModelChat
                ...
                onConversation={(c) => { setConversation(c); persist({ chat: c }); }}
              />
              <Memory
                notes={notes}
                onNotes={setNotes}
                onSave={() => persist({ notes })}
              />
            </div>
          </div>
        )}
        ...
        {tab === "assets" && (
          <div className="panel"><div className="panel-body">
            <p className="placeholder">Asset-management views — portfolio construction, risk, and
            allocation.</p>
          </div></div>
        )}
        {tab === "about" && (
          <div className="panel"><div className="panel-body">
            ...
            work. Market data via the Massive API; a multi-provider model chat (free and paid) for
            building and comparing. React + Vite frontend, FastAPI backend, built to control_deck
            conventions.</p>
          </div></div>
        )}
      </main>
